...
<template>
  <CardContent class="flex h-[24.8rem] w-full px-0 py-1 align-middle">
    <div v-if="isEmpty(wallet.txHistory)" class="m-auto cursor-default space-y-4">
    ...
    <ScrollArea v-else class="w-full">
      <div class="space-y-6 px-6 text-sm">
        <div
          v-for="(tx, index) in wallet.txHistory"
        ...
        >
          ...
          <Separator v-if="index < wallet.txHistory.length - 1" />
        </div>
      </div>
    </ScrollArea>
  </CardContent>
